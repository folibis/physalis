...
// One shape as the engine receives it: what it looks like, and whatever that
// engine says a shape has. The editor never interprets `params` -- the keys
// come from IPhysicsEngine::shapeProperties(), and an engine reads back the
// ones it published. A key it has never heard of is simply not read.
struct ShapePart {
    QString name;
    ...
    // Not a physics setting: a rule watches this shape. Whatever the engine
    // needs switched on to report its contacts, it switches on itself.
    bool watchedByRules = false;
};
...
struct BodyDesc {
    ...
    // The editor's own switch, not the engine's: a disabled body is never
    // handed over at all.
    bool isEnabled = true;

    // Keyed by IPhysicsEngine::bodyProperties(), as ShapePart::params is.
    QVariantMap params;
};
...
struct WorldDesc {
    // How many scene units make a metre. The scene's own scale rather than a
    // physics setting: the editor draws, measures and exports with it.
    qreal pixelsPerMeter = 1000.0;

    // Everything the world itself is set up with -- gravity, the solver's
    // tuning, whether bodies may sleep -- keyed by
    // IPhysicsEngine::worldProperties(). Which of those exist, and what they
    // are called, is the engine's business.
    QVariantMap params;
};
